...
## 10. Implementation defaults applied (phase 1–4)

§7 のオープン質問はすべて **推奨案どおり** に解決して Phase 1–4 (#150, #151, #152, #153) を実装した。後続 RFC で撤回する場合のため、各決定の最終着地点を記録する。

| 質問 | 採用された既定 | 関連コミット / issue |
|---|---|---|
| Q-naming | `<name>-<label>` (例: `gitea-dex`) | `exposeServiceName` in `cmd/app/proxyservices.go`; #150 |
| Q-rollback | deploy 部分失敗時は自動 reverse rollback、409 `no_drain_target` は stderr 警告へ降格して続行、非 409 は中止 | `reverseRollbackBlocks` in `cmd/app/deploy.go`; #152 |
| Q-atomic | N+1 proxy service の非アトミック swap を許容 (README に明示) | §1 README multi-host ノート; #153 |
| Q-env | `.env.server` は全ブロック共通で共有 (`env_file` を root web のみに付ける方針ではなく "全ブロック同一 env_file" を採用 — 後続で per-block 分離する場合は `expose.env_file` を追加) | `composeOverrideFor` in `cmd/app/override.go`; #152 |
| Q-grpc | 本 RFC 外。`quickwit-otel` は HTTP OTLP (`:4318`) のみ。gRPC は別 RFC へ | §4.5, §1.3 |
| Q-version-gate | README 明記のみ。`conoha_version` スキーマキーは追加しない | README multi-host ノート; #153 |

user-initiated rollback (`conoha app rollback`) の 409 挙動は deploy 経路とは別物である点に注意:

- `app rollback --target=<label>` (単一): 409 を **fatal error** として返す — ユーザが明示的にその 1 ブロックを指定しているため、静かに警告するとサイレント失敗になる。
- `app rollback` (無指定、全ブロック): 409 は **stderr 警告に降格して続行**。残りのブロックは rollback される。非 409 エラーは警告しつつ続行するが、`firstErr` を記録してコマンドは非 0 終了する。
