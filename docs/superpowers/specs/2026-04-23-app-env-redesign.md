...
**proxy モード**:
- deploy 時、`/opt/conoha/<name>/.env.server` をスロット work dir にコピーせず、compose override で `env_file: /opt/conoha/<name>/.env.server` を web service に付与する。
...
services:
  <web>:
    env_file:
      - /opt/conoha/<name>/.env.server:ro   # 新規: 欠落時は compose が黙って無視
```

`env_file` の `:ro` サフィックスは compose v2.23+ 記法。未対応バージョン用の fallback は override を 2 行追加する分岐で対応 (実装フェーズで最終決定)。
...
3. `destroy` は `rm -rf /opt/conoha/<name>` で新位置を自動清掃。旧位置残骸の `rm -f /opt/conoha/${APP_NAME}.env.server` 明示削除は **残す** (destroy は legacy サーバの掃除経路も兼ねるため — `destroy.go` 既存コメント参照)。
...
- `cmd/app/deploy.go`:
  - `runProxyDeploy`: compose override 生成時に `env_file` エントリを条件付きで注入 (ファイル存在チェックは compose 側に任せるが、明示的に unlink 状態の時は注入しない)。
...
- `cmd/app/destroy.go`:
  - 変更なし (`rm -rf /opt/conoha/<name>` で新位置は自動対応。legacy `rm -f` は既存のまま残す)。
...
- v0.1.x → v0.2.x で旧位置の `.env.server` は "warning + 後方互換 read" だが、**v0.3.x で warning → error** に上げる予定を README に明記する。

## 6. 受け入れ基準

- [ ] proxy モードで `app env set KEY=VALUE` → `app deploy` の後、新しいスロットの web コンテナが KEY を見れる。
- [ ] no-proxy モードで挙動が実質変わらない (新位置に書き込み、deploy 時に `.env` merge)。
- [ ] 旧位置の `.env.server` が残っているサーバに対して、`app env list` で旧位置を読めつつ deprecation 警告が出る。
- [ ] `app env migrate <server>` で旧→新の移動が安全に完了する。
- [ ] `destroy` 後、新位置・旧位置とも残骸ゼロ。
- [ ] proxy モードの `maybeWarnProxyEnvMode` warning 出力テストが削除され、代わりに "next: app deploy" 案内が確認される。

## 7. オープン技術判断

- **`env_file: :ro` サフィックスの可搬性**: compose v2.23+ 限定。ConoHa VMI の compose は v2.35 系なので実質問題なし。最低バージョンを README で明記する。
- **欠落時の compose 挙動**: `env_file` で指定されたファイルが存在しないと compose は **エラーで起動失敗**する (v2.20+)。空の `.env.server` を init 時に `touch` で作成するか、override 生成時に存在チェックするか。**決定: 前者** (`app init` で `touch .env.server` する)。override 側は単純化。
...
- 前提: feat/proxy-deploy (#98) merged
